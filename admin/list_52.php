        ...
        <div class="table-responsive">
            ...
            while($res = $DB->fetch($rs)){
                echo '<tr><td  style="color:green;">'.$res['out_trade_no'].'<br>'.($res['trade_no']==""?"无":$res['trade_no']).'</td>'
                    ...
                    . '<br>下单时间：'.$res['benTime']
                    . '</td><td style="color:#4B0082">联系QQ：'.$res['rel'].'<br>充值账号：'.$res['qkpb'].'<br>充值密码：'.$res['jnyt'].'<br>充值成功发到QQ邮箱通知：'.($res['sta']==1?'<a href="./list_aa.php?trade_no='.$res['trade_no'].'&gid='.$res['gid'].'&out_trade_no='.$res['out_trade_no'].'"><span class="btn btn-xs btn-primary ">发送</span> </a>':'未付款').'</tr>';
            }
        ?>
        </tbody>
    </table>
