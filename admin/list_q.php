        ...
        <div class="table-responsive">
            ...
            <table class="table layui-table">
                <thead>
                    <tr>
                        ...
                        <th style="color:#4B0082">QQ/付款账号</th>
            ...
            while($res = $DB->fetch($rs)){
                echo '<tr><td  style="color:green;">'.$res['out_trade_no'].'<br>'.($res['trade_no']==""?"无":$res['trade_no']).'</td>'
                    . '<td style="color:black">(#'.$res['gid'].')'.substr_utf8(getName($res['gid'],$DB),10).'</td><td style="color:#4B0082">'.$res['rel'].'<br>'.($res['phone']==""?$res['fcwfid']:$res['phone']).'</td><td style="color:	#3A5FCD">'.($res['paypass']==""?"未设置":$res['paypass']).'<br><font color=red>'.($res['coupon_id']==0?"无券":getcoupon_ka($res['coupon_id'])).'</font></td>'
                    . '<td style="color:#1874CD;">'.$res['benTime']."<br><font color=green>".($res['endTime']==""?"未知":$res['endTime']).'<font></td><td style="color:#00688B">'.$res['number'].'</td><td  style="color:#CD3333">￥'.$res['money'].'('.getPayType($res['type']).')'.'</td><td>'. ($res['sta']==1?'<a href="/safwl/m.php?trade_no='.$res['trade_no'].'&gid='.$res['gid'].'&out_trade_no='.$res['out_trade_no'].'"><span class="btn btn-xs btn-primary ">通知</span> </a>':'未完成').'</td>'
                    . '<td> <a href="./q.php?out_trade_no='.$res['out_trade_no'].'"><span class="btn btn-xs btn-primary ">发货</span> </a> </td></tr>';
            }
        ?>
        </tbody>
    </table>
